...
const player = new Player({ room: 1 });
...
test('taking object in room moves it to inventory', () => {
  ...
  take.action({ player, world, terminal }, 'take lantern'.match(take.pattern));
...
test('taking an item that fits the description of two items in the room results in a warning', () => {
  ...
  take.action({ player, world, terminal }, 'take brass'.match(take.pattern));
